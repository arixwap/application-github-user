...
import com.arixwap.dicoding.applicationgithubuser.databinding.ItemRowUserBinding
...
class ListUserAdapter(private val users: List<User>) :
    ...
    override fun onBindViewHolder(holder: ListViewHolder, position: Int) {
        ...
        holder.binding.run {
            ...
            locationItemUser.visibility =
                if (users[position].location == null) View.GONE else View.VISIBLE

            companyItemUser.text = users[position].company
            companyItemUser.visibility =
                if (users[position].company == null) View.GONE else View.VISIBLE
        }
        ...
        holder.binding.imgItemUser.setOnClickListener {
            itemClickCallback.onItemClicked(users[position].login!!)
        }
    }
